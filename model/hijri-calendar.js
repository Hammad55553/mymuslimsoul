...
async function fetchGregorianToHijriFullDate(date) {
  ...
  // Correct coordinates for Hasilpur, Pakistan
  let location = { latitude: 29.691135, longitude: 72.552776 };
  try {
    let response = await fetch(
      `https://api.aladhan.com/v1/timings/${fullDate}?latitude=${location.latitude}&longitude=${location.longitude}&method=1`
    );
...
async function getGregorianHijriFullDate(date) {
  ...
  return {
    ...
    hijri: {
      ...
      weekday: gregorianHijriDate.date.hijri.weekday.ar,
      month: {
